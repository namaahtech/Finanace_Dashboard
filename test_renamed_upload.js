...
async function run() {
  ...
  console.log("Test: Uploading test.ex_ file...");
  const result = await uploadToZoho(token, accountId, "test.ex_", Buffer.alloc(100 * 1024, 'a'), "application/octet-stream");
  console.log("Status:", result.status, "JSON:", JSON.stringify(result.json));
}
run();
