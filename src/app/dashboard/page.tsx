...
export default function EmployeeDashboard() {
  ...
  const stats = [
    ...
    { label: "Aggregate Yield", value: formatCurrency(wallet?.earned_total ?? 0), icon: <IndianRupee size={18} className="text-slate-600" />, trend: "Lifetime", color: "slate" },
  ];
    ...
    >
      <div className="space-y-10">
          ...
          {stats.map((s) => (
            <div key={s.label} className="page-card !p-6">
              <div className="flex justify-between items-start mb-4">
                 ...
                 <span className="text-[10px] font-bold text-slate-400 border border-slate-200 px-2 py-0.5 rounded uppercase">{s.trend}</span>
              </div>
              <div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
           {/* Multiplier Stack */}
           <div className="page-card p-10 space-y-10 border-none bg-slate-900 text-white shadow-2xl">
              <div>
                 <p className="text-[10px] font-bold uppercase tracking-[0.3em] text-slate-400 mb-2">Total Multiplier Stack</p>
              ...
              <div className="grid grid-cols-2 gap-8 border-t border-slate-800 pt-10">
                 <div>
                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2">Personal Factor</p>
                    <p className="text-2xl font-bold text-emerald-400 tracking-tight">{employeeMultiplier.toFixed(1)}x</p>
                    <p className="text-[10px] text-slate-500 mt-1">Based on {employeeScore}% score</p>
                 </div>
                 <div>
                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2">Company Factor</p>
                    <p className="text-2xl font-bold text-blue-400 tracking-tight">{companyMultiplier.toFixed(1)}x</p>
                    <p className="text-[10px] text-slate-500 mt-1">Based on {companyScore}% health</p>
                 </div>
              </div>

              <div className="bg-white/5 rounded-lg p-6 border border-white/10 flex justify-between items-center">
                 <div>
                    <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400 mb-1">Authorised Payout</p>
           ...
           <div className="page-card p-10 flex flex-col justify-between">
              <div>
                 <h3 className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.3em] mb-6">Mission Status</h3>
                 <p className="text-sm text-slate-600 leading-relaxed max-w-sm">
                    Your current trajectory is <span className="text-slate-900 font-bold uppercase tracking-tight">high performance</span>. 
              ...
              <div className="space-y-6 pt-10">
                 <div className="flex justify-between items-center border-b border-slate-100 pb-4">
                    ...
                    <span className="text-xs font-bold text-slate-900">11 May 2026</span>
                 </div>
                 <div className="flex justify-between items-center border-b border-slate-100 pb-4">
                    <span className="text-xs font-medium text-slate-500 flex items-center gap-2">
                       <IndianRupee size={14} /> Base Component
                    </span>
                    <span className="text-xs font-bold text-slate-900">{formatCurrency(baseIncentive)}</span>
                 </div>
                 <div className="flex justify-between items-center pb-4">
                    <span className="text-xs font-medium text-slate-500 flex items-center gap-2">
                       <CalendarDays size={14} /> Tracking Cycle
                    </span>
                    <span className="text-xs font-bold text-slate-900">Current Q2</span>
                 </div>
              </div>
           </div>
        </div>
