    ...
    <DashboardShell
      ...
      actions={
        <div className="flex gap-3">
          <div className="flex bg-slate-900 rounded-xl p-1 shadow-lg border border-slate-800">
            <button className="px-4 py-2 bg-slate-800 text-white text-[10px] font-black uppercase tracking-widest rounded-lg transition-all">
              Live Feed
            </button>
            <button className="px-4 py-2 text-slate-400 hover:text-white text-[10px] font-black uppercase tracking-widest rounded-lg transition-all">
              History
            </button>
          </div>
          <button className="flex items-center gap-2 px-6 py-2.5 bg-white text-slate-900 text-[10px] font-black uppercase tracking-widest rounded-xl hover:bg-slate-100 transition-all shadow-xl">
            <Calendar size={12} />
            Export Report
          </button>
        </div>
    ...
    >
      <div className="flex flex-col gap-6">
          ...
          {ROLES.map((role) => (
            ...
            >
              ...
              <div className={cn(
                ...
                selectedRole === role ? "bg-white/10" : "bg-slate-100"
              )}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 group enterprise-card bg-slate-900 border-slate-800 p-8 flex flex-col justify-between overflow-hidden relative">
             ...
             <div>
                <div className="flex items-center gap-2 mb-8">
                   ...
                   <span className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em]">Sector Performance {selectedRole}</span>
                </div>
                
                <div className="grid grid-cols-2 gap-12 mb-12">
                   <div>
                      <h2 className="text-5xl font-black text-white tracking-tighter mb-2">{stats.attendance}%</h2>
                      <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest leading-loose">Daily Attendance Velocity</p>
                   ...
                   <div>
                      ...
                      <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest leading-loose">Log Sheet Progress Rail</p>
                      <div className="mt-6 h-1 w-full bg-slate-800 rounded-full overflow-hidden">
             ...
             <div className="flex gap-8 border-t border-slate-800 pt-8">
                <div>
                   <span className="block text-[8px] font-black text-slate-500 uppercase tracking-widest mb-1">Total Unit Headcount</span>
                   <span className="text-xl font-black text-white">{stats.employees} Personnel</span>
                </div>
                <div className="w-px bg-slate-800" />
                <div>
                   <span className="block text-[8px] font-black text-slate-500 uppercase tracking-widest mb-1">Status: Operational</span>
                   <span className="text-xl font-black text-white">{stats.active} On-Field</span>
                </div>
             </div>
          </div>

          <div className="enterprise-card bg-white p-8 flex flex-col justify-between group">
             <div className="mb-8">
                <div className="flex items-center justify-between mb-8">
                   <h3 className="text-[10px] font-black text-slate-900 uppercase tracking-widest">Compliance Drift</h3>
                   <div className="p-2 bg-slate-100 rounded-lg group-hover:bg-slate-900 group-hover:text-white transition-colors">
                      <ArrowUpRight size={16} />
                   </div>
                </div>
                <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest leading-relaxed mb-6">Real-time monitoring of policy adherence across decentralized units.</p>
                   ...
                   {[1, 2, 3].map(i => (
                     <div key={i} className="flex flex-col gap-2">
                        ...
                        <div className="h-1 w-full bg-slate-100 rounded-full overflow-hidden">
                           <div className="h-full bg-slate-900" style={{ width: `${90 + i}%` }} />
                        </div>
                     </div>
                   ))}
                </div>
             </div>
             <button className="w-full py-4 text-[10px] font-black text-slate-900 uppercase tracking-widest border border-slate-100 rounded-xl hover:bg-slate-900 hover:text-white transition-all">
        ...
        <div className="enterprise-card bg-white overflow-hidden p-0">
            ...
            <div className="flex items-center gap-4">
              <div className="relative">
                ...
                <input 
                  ...
                  className="pl-10 pr-4 py-2 bg-slate-50 border-none rounded-lg text-[9px] font-black uppercase tracking-widest focus:ring-1 focus:ring-slate-200 outline-none w-64"
                />
              </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left">
              <thead>
                <tr className="bg-slate-50 text-[9px] text-slate-500 font-black uppercase tracking-[0.2em]">
              ...
              <tbody className="divide-y divide-slate-50">
                {EMPLOYEES.map((person) => (
                  <tr key={person.id} className="group hover:bg-slate-50 transition-colors cursor-pointer">
                    <td className="px-8 py-4">
                      <div className="flex items-center gap-3">
                        ...
                        <span className="text-xs font-black tracking-tight text-slate-900">{person.name}</span>
                      </div>
                    </td>
                    <td className="px-8 py-4">
                      <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">{person.role} Unit</span>
                    </td>
                    <td className="px-8 py-4 font-mono text-[10px] text-slate-400">{person.clockIn}</td>
                    <td className="px-8 py-4 font-mono text-[10px] text-slate-400">{person.clockOut}</td>
                    <td className="px-8 py-4 font-mono text-[10px] text-slate-400">{person.hours} hrs</td>
                    <td className="px-8 py-4 text-xs font-black text-slate-900">{person.compliance}%</td>
          ...
          <div className="p-6 bg-slate-50 border-t border-slate-50 flex items-center justify-center">
            <button className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] hover:text-slate-900 transition-colors flex items-center gap-2">
