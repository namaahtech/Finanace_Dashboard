const fs = require('fs');
const path = require('path');
const { createClient } = require('@supabase/supabase-js');

const envPath = path.join(__dirname, '..', '.env.local');
const envText = fs.readFileSync(envPath, 'utf8');
const env = {};
envText.split(/\r?\n/).forEach(line => {
  const cleanLine = line.trim();
  if (!cleanLine || cleanLine.startsWith('#')) return;
  const parts = cleanLine.split('=');
  if (parts.length >= 2) {
    const key = parts[0].trim();
    let val = parts.slice(1).join('=').trim();
    if ((val.startsWith('"') && val.endsWith('"')) || (val.startsWith("'") && val.endsWith("'"))) {
      val = val.slice(1, -1);
    }
    env[key] = val;
  }
});

const supabase = createClient(env.NEXT_PUBLIC_SUPABASE_URL, env.SUPABASE_SERVICE_ROLE_KEY);

async function run() {
  console.log("Checking teams table columns...");
  const { data: cols, error: colErr } = await supabase.rpc('exec_sql', {
    sql_query: `
      SELECT column_name, data_type 
      FROM information_schema.columns 
      WHERE table_name = 'teams';
    `
  });
  
  if (colErr) {
    console.log("No exec_sql, querying select limit 1 on teams...");
    const { data: teams, error } = await supabase.from("teams").select("*").limit(1);
    if (error) console.error("Error teams:", error);
    else console.log("Teams columns:", Object.keys(teams[0] || {}));
  } else {
    console.log("Teams cols:", cols);
  }

  console.log("Checking employees table columns...");
  const { data: emp, error: empErr } = await supabase.from("employees").select("*").limit(1);
  if (empErr) console.error("Error employees:", empErr);
  else console.log("Employees columns:", Object.keys(emp[0] || {}));
}
run();
